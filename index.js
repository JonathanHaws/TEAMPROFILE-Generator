...
function createCard(employee, infotextcolor, headercolor, infocolor, headertextcolor){ 
    ...
    let cardstyle       = 'width:min-width; margin:auto; padding-top:20px; padding-left:10px; padding-right:10px'
    let headerstyle     = 'background-color:'+ headercolor + '; border-radius:12px 12px 0px 0px;';
    let infostyle       = 'background-color:'+ infocolor + '; border-radius:0px 0px 12px 12px; padding:10px;'
    ...
    switch (employee.getRole()){
        ...
        case 'Engineer': info += '<a href="https://github.com/'+ employee.getGithub() + '">Github: ' + employee.getGithub()+ '</a>';  break;
...
async function createTeamProfile(style){
    ...
    htmldocument += '    <div style = "margin:0px; padding:10px; background-color: ' + style.colors[1] + '"> <h1 style="color:'+ style.colors[2] +'"> Team Profiles </h1></div>\n'
...
createTeamProfile({colors:['#2b2b2b','#b85f4d','#c2c2c2','#5c5c5c']})
